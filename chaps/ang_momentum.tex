...
\begin{linenomath}
    ...
    \end{equation}
\end{linenomath}
is the reduced (Wigner small) d-matrix. The factorization follows because $J_z$ is diagonal in the $\ket{j,m}$ basis---the first and third Euler factors contribute only the phases $e^{-i\alpha m'}$ and $e^{-i\gamma m}$---while the middle factor, the rotation about the $y$-axis, mixes $m$ values and carries the nontrivial $\beta$-dependence.

The physical meaning of the D-matrices is that they are the $(2j+1)$-dimensional irreducible unitary representations of SU(2). Under a rotation $R$, an angular momentum basis state transforms as
\begin{linenomath}
    \begin{equation}
        U(R)\ket{j,m} = \sum_{m'=-j}^{j} D^{(j)}_{m'm}(R)\,\ket{j,m'}.
    \end{equation}
\end{linenomath}
Every member of a $j$-multiplet is carried into a linear combination of all members, with the D-matrix supplying the coefficients. The D-matrices are the finite-rotation counterpart of the ladder coefficients of Section~\ref{sec:angular-momentum-algebra}: the ladders step by one unit of $m$, the D-matrices rotate the entire multiplet by an arbitrary angle.

The D-matrices obey a set of fundamental properties that the remainder of the chapter spends.

\medskip\noindent\textbf{Composition.} From $U(R_1R_2)=U(R_1)U(R_2)$, the matrix elements satisfy
\begin{linenomath}
    \begin{equation}
        \sum_{m''=-j}^{j} D^{(j)}_{m'm''}(R_1)\,D^{(j)}_{m''m}(R_2)
        = D^{(j)}_{m'm}(R_1R_2),
    \end{equation}
\end{linenomath}
which is the statement that the $D^{(j)}$ are group representations: the matrix of the product is the product of the matrices.

\medskip\noindent\textbf{Orthogonality.} The D-matrices for inequivalent representations are orthogonal under integration over the group manifold. With the invariant integration measure $dR = d\alpha\,d\cos\beta\,d\gamma$ over the Euler-angle domain,
\begin{linenomath}
    \begin{equation}
        \int dR\; D^{(j)*}_{m'm}(R)\,D^{(j')}_{\mu'\mu}(R)
        = \frac{8\pi^2}{2j+1}\,\delta_{jj'}\,\delta_{m\mu}\,\delta_{m'\mu'}.
    \end{equation}
\end{linenomath}
This is the great orthogonality theorem for SU(2). The proof---which rests on Schur's lemma and the invariance of the Haar measure---is deferred to Appendix~B; the physics consequences (the orthogonality of states transforming under inequivalent representations and the completeness of the D-matrices as a basis for square-integrable functions on the group, the Peter--Weyl theorem) are developed here.

\medskip\noindent\textbf{Complex conjugation.} From the unitarity of $U(R)$ and the Condon--Shortley phase convention for the $d$-matrix elements,
\begin{linenomath}
    \begin{equation}
    ...
    \end{equation}
\end{linenomath}
The relation is proved by explicit conjugation of \eqref{eq:D-matrix-def}:
\begin{linenomath}
\begin{align*}
D^{(j)*}_{m'm}(\alpha,\beta,\gamma)
&= \bigl(e^{-i\alpha m'} d^{(j)}_{m'm}(\beta) e^{-i\gamma m}\bigr)^*
 = e^{i\alpha m'} d^{(j)*}_{m'm}(\beta) e^{i\gamma m}.
\end{align*}
\end{linenomath}
The Condon--Shortley convention fixes the $d$-matrix elements to be real and endows them with the symmetry $d^{(j)*}_{m'm}(\beta) = (-1)^{m-m'} d^{(j)}_{-m',-m}(\beta)$ (a consequence of the real, positive ladder matrix elements in \eqref{eq:ladder-action-j}). Substituting,
\begin{linenomath}
\begin{align*}
D^{(j)*}_{m'm}(\alpha,\beta,\gamma)
&= (-1)^{m-m'} e^{i\alpha m'} d^{(j)}_{-m',-m}(\beta) e^{i\gamma m}
 = (-1)^{m-m'} D^{(j)}_{-m',-m}(\alpha,\beta,\gamma),
\end{align*}
\end{linenomath}
since $D^{(j)}_{-m',-m}(\alpha,\beta,\gamma)=e^{-i\alpha(-m')}d^{(j)}_{-m',-m}(\beta)e^{-i\gamma(-m)}=e^{i\alpha m'}d^{(j)}_{-m',-m}(\beta)e^{i\gamma m}$, matching the conjugated expression.

\medskip\noindent\textbf{Connection to spherical harmonics.} For integer $\ell$, the $D$-matrices reduce to spherical harmonics. The column $m'=0$ of $D^{(\ell)}$, evaluated at $\gamma=0$, gives
\begin{linenomath}
    \begin{equation}
    ...
    \end{equation}
\end{linenomath}
In particular, $D^{(\ell)}_{00}(\alpha,\beta,\gamma) = P_\ell(\cos\beta)$, the Legendre polynomial, independent of $\alpha$ and $\gamma$ as required for a scalar under rotations about the $z$-axis. The spherical harmonics are therefore a special case of the rotation matrices: the functions on the sphere $S^2 \cong \mathrm{SO}(3)/\mathrm{SO}(2)$ that transform under the $\ell$-representation are precisely the $m'=0$ column of the D-matrix. This connection is developed fully in Section~\ref{sec:orbital-angular-momentum}; here it is stated to exhibit the unity of the finite-rotation formalism: the D-matrices contain the spherical harmonics as a special case, just as they contain the spin-$1/2$ amplitudes as the $j=1/2$ instance.

\medskip\noindent\textbf{Explicit form for $j=1/2$.} The fundamental representation is read off directly from the Euler decomposition. For $j=1/2$, $J_i = (\hbar/2)\sigma_i$, and
\begin{linenomath}
    \begin{equation}
        U(\alpha,\beta,\gamma) = e^{-i\alpha\sigma_z/2}\,e^{-i\beta\sigma_y/2}\,e^{-i\gamma\sigma_z/2}.
    \end{equation}
\end{linenomath}
The reduced d-matrix in the $\{\ket{+z},\ket{-z}\}$ basis is the middle factor:
\begin{linenomath}
    \begin{equation}
        d^{(1/2)}(\beta) = e^{-i\beta\sigma_y/2}
        = \begin{pmatrix}
            \cos(\beta/2) & -\sin(\beta/2) \\[2pt]
            \sin(\beta/2) &  \cos(\beta/2)
        \end{pmatrix}.
    \end{equation}
\end{linenomath}
The full D-matrix follows by sandwiching with the diagonal $z$-rotation factors:
\begin{linenomath}
    \begin{equation}
    ...
    \end{equation}
\end{linenomath}
The half-angle $\beta/2$ is the signature of the double cover: the matrix entries are $4\pi$-periodic in $\alpha$ and $\gamma$, and $D^{(1/2)}(\alpha,\beta,\gamma+2\pi) = -D^{(1/2)}(\alpha,\beta,\gamma)$. The first column, evaluated at $\gamma=0$, is exactly the spin-$\bm n$ state $\ket{+\bm n}$ that Section~\ref{sec:spin-half} will construct from the reference state $\ket{+z}$ by the Euler rotation $U(\alpha,\beta,0)$.

\medskip\noindent\textbf{Example: spin precession re-read through Euler angles.} The Larmor precession of the evolution chapter (Section~\ref{sec:spin-precession}) acquires its Euler-angle reading here. The evolution operator for a spin in a uniform magnetic field $\bm B = B\hat{z}$ is
\begin{linenomath}
    \begin{equation}
        U(t) = e^{-i\omega t\,\sigma_z/2},
    \end{equation}
\end{linenomath}
with $\omega = -\gamma B$ (\eqref{eq:larmor-evolution}). In the Euler-angle language of \eqref{eq:euler-angles}, this is the special case $\alpha = \omega t$, $\beta = 0$, $\gamma = 0$: a one-parameter subgroup generated by $J_z$ alone. The D-matrix element \eqref{eq:D-matrix-def} gives the amplitude for the state to remain in the initial $m$-state:
\begin{linenomath}
    \begin{equation}
        D^{(1/2)}_{++}(\omega t,0,0) = e^{-i\omega t/2},
        \qquad
        D^{(1/2)}_{--}(\omega t,0,0) = e^{i\omega t/2}.
    \end{equation}
\end{linenomath}
At $t = 2\pi/\omega$ the D-matrix is $D^{(1/2)}(2\pi,0,0) = -I$, exactly the spinor sign that the evolution chapter computed (\eqref{eq:spinor-sign}): the state has traversed a full cycle in SU(2) while the spatial rotation in SO(3) is the identity. The expectation vector $\langle\bm\sigma\rangle(t)$ of \eqref{eq:precession} rotates at the frequency $\omega$ in $\mathbb{R}^3$:
\begin{linenomath}
    \begin{equation}
        \langle\sigma_x\rangle(t) = \langle\sigma_x\rangle(0)\cos\omega t - \langle\sigma_y\rangle(0)\sin\omega t,
        \qquad
        \langle\sigma_y\rangle(t) = \langle\sigma_x\rangle(0)\sin\omega t + \langle\sigma_y\rangle(0)\cos\omega t.
    \end{equation}
\end{linenomath}
The D-matrix element $D^{(1/2)}_{++}(t) = e^{-i\omega t/2}$ gives the state evolution directly: the state rotates at half the SO(3) rate because $J_z = (\hbar/2)\sigma_z$ has eigenvalue $+\hbar/2$, and the Euler angle $\alpha = \omega t$ is the spatial rotation angle. The factor $1/2$ in the exponent is the $m=1/2$ eigenvalue in the phase $e^{-i\alpha m}$ of \eqref{eq:D-matrix-def}. The 2:1 covering is made quantitative: the expectation vector (SO(3)) completes one cycle at $\omega t = 2\pi$, while the state (SU(2)) completes one cycle at $\omega t = 4\pi$.

The advantage of the Euler-angle and D-matrix reading is that it generalizes immediately to arbitrary rotations: the D-matrix \eqref{eq:D-matrix-def} gives the amplitude for any initial state $\ket{j,m}$ to be found in any final state $\ket{j,m'}$ after any rotation parametrized by $(\alpha,\beta,\gamma)$, and the composition property converts sequences of rotations into matrix multiplication. The D-matrices are the finite-rotation machinery the remainder of the chapter spends. Its limitation is that the D-matrices are defined on the angular momentum basis, which Section~\ref{sec:angular-momentum-algebra} must still construct; the group-theoretic properties stated here (the great orthogonality theorem and the Peter--Weyl completeness) are named with Appendix~B pointers---their proof requires Schur's lemma and the invariance of the Haar measure, which belong to representation theory, not to physics. The sections that follow need only the algebra and the explicit D-matrix forms, not the general theory.

The finite-rotation formalism---the group structure, the 2:1 covering, the Euler-angle parametrization, and the Wigner D-matrices---is now complete. The next question is algebraic: what eigenvalues can the generators $J^2$ and $J_z$ possess? The answer is independent of the group topology and follows from the single commutator \eqref{eq:galilei-algebra} alone, by the ladder method of Chapter~5.
...
\begin{linenomath}
  ...
  \end{equation*}
\end{linenomath}
The 6-$j$ symbol is a recoupling coefficient, not a coupling coefficient; it is a number (independent of all $m$'s) that encodes the geometry of three angular momenta. Its physical context is the transformation between the $LS$-coupling and $jj$-coupling schemes of atomic spectroscopy: in $LS$ coupling the orbital angular momenta are coupled first, then the spins, then the totals; in $jj$ coupling each electron's orbital and spin are coupled first, then the individual $j$'s are coupled to the total $J$. The 6-$j$ symbol provides the unitary matrix connecting the two descriptions. The full Racah algebra of $3nj$-symbols -- 6-$j$, 9-$j$, and their symmetries -- is developed in Appendix~B.
