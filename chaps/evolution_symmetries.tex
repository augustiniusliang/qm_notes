...
\end{equation}
\end{linenomath}
The internal Hamiltonian $H_{\mathrm{int}}$ must be a Galilean scalar: it commutes with every generator of the spacetime algebra. For a particle without internal structure, $H_{\mathrm{int}} = \mathrm{const}$, and the free-particle form \eqref{eq:free-hamiltonian} is recovered. The proof is the computation of \eqref{eq:free-hamiltonian}, restated:
\begin{linenomath}
\begin{equation*}
\bigl[K_i,\,H - \tfrac{\bm P^{2}}{2M}\bigr] = -i\hbar P_i - \tfrac{1}{2M}((-i\hbar\delta_{ij}M)P_j + P_j(-i\hbar\delta_{ij}M)) = 0 .
\end{equation*}
\end{linenomath}
The same quantity commutes with $\bm P$ and with $\bm J$, so it is a central element and, for an irreducible representation, proportional to the identity --- exactly $E_{\mathrm{int}}$ in \eqref{eq:free-hamiltonian}.
